    ...
    if ($method == 'POST') {
        ...
        if(empty($userid)||empty($logisticid)||empty($deliveryAddressid)||empty($shipType)|| empty($items)){
            ...
            for($i = 0; $i < count($items); $i ++){
                ...
                array_push($itemPrices, array("price"=>$sellingPrice, "orderPrice"=>$orderPrice));
            }
            ...
            $logistic = getLogisticPrice($connect, $logisticid);
            if(!$logistic){
            ...
            $minWeight = $logistic['lbsweightmin'];
            $maxWeight = $logistic['lbsweightmax'];
            ...
            if($totalWeight < $minWeight && $totalWeight > $maxWeight){
                ...
                $errordata=returnError7003($errordesc,$linktosolve,$hint);
                $text="weight not within logistic weight range";
                $method=getenv('REQUEST_METHOD');
                $data=returnErrorArray($text,$method,$endpoint,$errordata);
                respondBadRequest($data);
            }
            ...
            //generate order time
            $orderTime = time();
            $orderRefno = generateCart($connect,$userid,$logisticid,$deliveryAddressid,$deliveryCharge, $shipType,$totalWeight, $trackid,$noOfItem, $orderTime, $totalPaid);
            ...
            for($i = 0; $i < count($items); $i ++){
                //count item in an order to get totalNoOfItem
                $totalItem = $totalItem + $items[$i]->quantity;
                //get pricepaid for order to get totalPaid
                $orderPrice = $items[$i]->quantity * $itemPrices[$i]->price;
                $totalPaid = $totalPaid + $orderPrice;

                //insert into DB
                $orderStmt->bind_param("ssss",$userid,$orderRefno,$items[$i]->productid,$items[$i]->quantity, $itemPrices[$i]->price);

                $productStmt->bind_param('ssss', $items[$i]->quantity, $items[$i]->productid, $items[$i]->productid );
