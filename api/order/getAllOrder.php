    ...
    if($method =='GET'){
        ...
        if (!empty($search) && $search!="" && $search!=' '){
            ...
            $searchQuery = "SELECT productorders.id, productorders.user_id, users.fname AS firstname, users.lname AS lastname, productorders.order_refno, productorders.product_id, products.name AS product_name, products.weight, productorders.price, productorders.quantity
                            ...
                            WHERE ( productorders.order_refno LIKE ? OR products.name like ? OR users.fname LIKE ? OR users.lname LIKE ?)";
            ...
            $searchQuery = "SELECT productorders.id, productorders.user_id, users.fname AS firstname, users.lname AS lastname, productorders.order_refno, productorders.product_id, products.name AS product_name, products.weight, productorders.price, productorders.quantity
                            FROM `productorders` LEFT JOIN users ON productorders.user_id = users.id LEFT JOIN products ON productorders.product_id = products.id
                            WHERE ( productorders.order_refno LIKE ? OR products.name like ? OR users.fname LIKE ? OR users.lname LIKE ?) ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($searchQuery);
            ...
            $sqlQuery = "SELECT productorders.id, productorders.user_id, users.fname AS firstname, users.lname AS lastname, productorders.order_refno, productorders.product_id, products.name AS product_name, products.weight, productorders.price, productorders.quantity
                        FROM `productorders` LEFT JOIN users ON productorders.user_id = users.id LEFT JOIN products ON productorders.product_id = products.id  ORDER BY id DESC";
            ...
            $sqlQuery = "SELECT productorders.id, productorders.user_id, users.fname AS firstname, users.lname AS lastname, productorders.order_refno, productorders.product_id, products.name AS product_name, products.weight, productorders.price, productorders.quantity
                        FROM `productorders` LEFT JOIN users ON productorders.user_id = users.id LEFT JOIN products ON productorders.product_id = products.id  ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($sqlQuery);
        ...
        if($numRow > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                array_push($allResponse,array("id"=>$id,"orderRefno"=>$orderRefno, "userid"=>$userid,"firstname"=>$firstname,"lastname"=>$lastname,"productid"=>$productid,"productName"=>$productName, "price"=>$price, "weight"=>$weight, "quantity"=>$quantity));
            }
            $maindata = [
                'page' => $page_no,
                'per_page' => $noPerPage,
