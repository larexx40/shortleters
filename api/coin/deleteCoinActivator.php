<?php
    ...
    if ($method == 'POST') {
        ...
        $sqlQuery = 'DELETE FROM coinactivators WHERE id = ?';
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = "Coin Activator has been Deleted in the database";
            $errordata = [];
            $text = "Coin activator successfuully deleted";
            $status = true;
            $data = returnSuccessArray($text, $method, $endpoint, $errordata, $maindata, $status);
            respondOK($data);
        }else{
            //id not found response
            $errordesc = "Coin activator id not found";
            $linktosolve = 'https://';
