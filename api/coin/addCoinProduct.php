<?php
    ...
    if($method =='POST'){
        ...
        if(!is_numeric($istype)){
            ...
            $errordesc = "is type should be 0 or 1";
            $linktosolve = 'https://';
        ...
        $status =1;
        //autogenerate producttrackid
        $producttrackid = generateUniqueShortKey($connect,'coinproducts','producttrackid');
        if(empty($producttrackid) || empty($name)||empty($rate)||empty($cointype)||empty($img) || empty($typetag) ||empty($colorcode) ||empty($wheretoshow) ||empty($platformtype) ||empty($liveratefunctions)){
            ...
            $sqlQuery = "INSERT INTO `coinproducts`(`producttrackid`, `name`, `merchantid`, `rate`, `cointype`, `status`, `img`, `colorcode`, `typetag`, `istype`, `wheretoshow`, `platformtype`, `liveratefunctions`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $connect->prepare($sqlQuery);
            $stmt->bind_param("sssssssssssss",$producttrackid, $name, $merchantid, $rate, $cointype, $status,$img, $colorcode, $typetag, $istype, $wheretoshow, $platformtype, $liveratefunctions);
            $insertCoinProduct=$stmt->execute();

            if($insertCoinProduct){
                ...
                $hint = [];
                $errordata = [];
                $text = "Coin Product Inserted Successfully";
                $status = true;
