<?php
    ...
    if($method =='GET'){
        ...
        $sqlQuery = "DELETE FROM `logistics_prices` WHERE id = ? AND logistic_id = ?";
        $stmt = $connect->prepare($sqlQuery);
        $stmt->bind_param("ss",$logisticPriceid, $logisticsid);
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = ["Location price deleted"];
            $errordata = [];
            $text = "location Price successfully deleted";
            $status = true;
            $data = returnSuccessArray($text, $method, $endpoint, $errordata, $maindata, $status);
            respondOK($data);
        }else {
            //id not found response
            $errordesc="Location price not found";
            $linktosolve="htps://";
            $hint="Price id not found";
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="location price id not found";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
