    ...
    if($method =='GET'){
        ...
        if (!empty($search) && $search!="" && $search!=' '){
            ...
            $searchQuery = "SELECT logistic_locations.id,logistic_locations.name AS location_name, logistic_locations.logistic_id, logistics.name AS logistic_name, 
                            ...
                            WHERE (logistic_locations.name like ? OR logistics.name like ? ) AND logistic_locations.status = ?";
            ...
            $searchQuery = "SELECT logistic_locations.id,logistic_locations.name AS location_name, logistic_locations.logistic_id, logistics.name AS logistic_name, 
                            logistic_locations.longitude,logistic_locations.latitude, logistic_locations.status
                            FROM logistic_locations LEFT JOIN logistics ON logistic_locations.logistic_id = logistics.id 
                            WHERE (logistic_locations.name like ? OR logistics.name like ? ) AND logistic_locations.status = ? ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($searchQuery);
            $stmt->bind_param("sssss", $searchParam, $searchParam, $offset, $noPerPage, $status);
            $stmt->execute();
            $result= $stmt->get_result();
            ...
            $sqlQuery = "SELECT logistic_locations.id,logistic_locations.name AS location_name, logistic_locations.logistic_id, logistics.name AS logistic_name, 
                        logistic_locations.longitude,logistic_locations.latitude, logistic_locations.status
                        FROM logistic_locations LEFT JOIN logistics ON logistic_locations.logistic_id = logistics.id 
                        WHERE logistic_locations.status = ? ORDER BY id DESC";
            ...
            $sqlQuery = "SELECT logistic_locations.id,logistic_locations.name AS location_name, logistic_locations.logistic_id, logistics.name AS logistic_name, 
                        logistic_locations.longitude,logistic_locations.latitude, logistic_locations.status
                        FROM logistic_locations LEFT JOIN logistics ON logistic_locations.logistic_id = logistics.id 
                        WHERE logistic_locations.status = ? ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($sqlQuery);
        ...
        if($numRow > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                array_push($allResponse, array("id"=>$id, "locationName"=>$locationName, "logisticid"=>$logisticid, "logisticName"=>$logisticName, "initials"=>$initials, "latitude"=>$latitude, "longitude"=>$longitude, "status"=>$status));
            }
