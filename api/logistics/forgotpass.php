<?php
    ...
    if ($method == 'POST') {
        ...
        if (!isset($_POST['username'])){
            ...
            $query = 'SELECT * FROM logistics WHERE shop_email = ? OR office_phone = ?';
            ...
            if ( $num_row < 1){
                ...
                $tokenQuery = 'INSERT INTO token (user_id, useridentity, token, time, verifytype, user_type) Values (?, ?, ?, ?, ?, ?)';
                $tokenStmt = $connect->prepare($tokenQuery);
                $tokenStmt->bind_param("issiii", $user_id, $username, $token, $expiretime, $verifyType, $user_type);


                // check if statement executes 
                if ($tokenStmt->execute()){
                    $tokenStmt->close();
                    $tokenlink = $logistics_resetLink. "?token=".$token;
