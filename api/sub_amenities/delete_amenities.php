<?php
    ...
    if ($method == 'POST') {
        ...
        $query = 'DELETE FROM `sub_amenities` WHERE `sub_amen_id` = ?';
        $slider_stmt = $connect->prepare($query);
        $slider_stmt->bind_param("s", $sub_amenity_id);
        $slider_stmt->execute();
        ...
        if ( $rows_affected > 0 ) {
            ...
            $text= "Sub Amenity successfully deleted";
            $status = true;
            $data = [];
            ...
            $errordesc = "Sub Amenity not Found";
            $linktosolve = 'https://';
