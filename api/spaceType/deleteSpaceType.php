    ...
    if($method =='POST'){
       ...
       if(!isset($_POST['spaceTypeid'])){
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = ["Space Type deleted from the database"];
            $errordata = [];
            $text = "Space Type successfully deleted";
            $status = true;
            $data = returnSuccessArray($text, $method, $endpoint, $errordata, $maindata, $status);
            respondOK($data);
        }else{
            //id not found response
            $errordesc="Space Type id not found";
            $linktosolve="htps://";
            $hint='Space Type id not in database';
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Space Type id not found";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
