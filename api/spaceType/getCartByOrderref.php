<?php
    ...
    if($method =='GET'){
        ...
        //everybody can acces this route

        //check if inpput is passed
        if(!isset($_GET['orderref'])){
            $errordesc="order ref required";
            $linktosolve="htps://";
            $hint=["Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Pass in your order ref";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
            
        }else {
            $orderRefno = cleanme($_GET['orderref']);    
        }        
        ...
        if(empty($orderRefno)){
            ...
            $hint=["Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Please pass in orderrefno";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
        ...
        $sqlQuery = "SELECT productcart.`id`,productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`, productcart.`orderref_number`, 
                    productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, deliveryaddress.address_no, deliveryaddress.address, deliveryaddress.phoneno, deliveryaddress.state, deliveryaddress.country, deliveryaddress.zipcode, productcart.assigntodeliveryman, productcart.seenbyseller,productcart.ordertime, productcart.`totalpaid`, productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge 
                    FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id LEFT JOIN deliveryaddress ON productcart.deliveryaddress_id = deliveryaddress.id WHERE productcart.orderref_number =?";
        ...
        if($numRow > 0){
            ...
            $grandTotal = $totalPaid + $deliveryCharge;
