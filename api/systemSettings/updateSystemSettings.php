<?php
    ...
    if ($method == 'POST') {
        ...
        if (!isset($_POST['activepaymentsystem'])){
            $errordesc = "activepaymentsystem must be passed";
            $linktosolve = 'https://';
            $hint = "Kindly pass the required activepaymentsystem field in this endpoint";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, []);
            respondBadRequest($data);
        }else{
            $activepaymentsystem = cleanme($_POST['activepaymentsystem']);
        }
