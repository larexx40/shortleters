<?php
    ...
    if($method =='POST'){
        ...
        $adminid =checkIfIsAdmin($connect, $adminpubkey);
         ...
         //confirm if systemSettingsid id is passed
        ...
        //confirm if systemSettingsid is not empty
        ...
        //delete Highlight
        $sqlQuery = "DELETE FROM `systemsettings` WHERE `sys_setting_id` =?";
        $stmt = $connect->prepare($sqlQuery);
        $stmt->bind_param("s",$systemSettingsid);
        $stmt->execute();
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = ["systemsettings deleted from the database"];
            $errordata = [];
            $text = "systemsettings successfully deleted";
            $status = true;
            ...
            $linktosolve="htps://";
            $hint='systemsettings with id not in database';
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="systemsettings with id not found";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
