<?php
    ...
    if ($method == 'POST') {
        ...
        if ( empty($apartment_facility_id) || empty($facility_id) || empty($apartment_id) ){
        ...
        $query = 'UPDATE `apartment_facilities` SET `apartment_id`= ?,`facility_id`= ?,`total_number`= ? WHERE apart_facility_id = ?';
        $slider_stmt = $connect->prepare($query);
        $slider_stmt->bind_param("ssss", $apartment_id, $facility_id, $number, $apart_facility_id);

        if ( $slider_stmt->execute() ) {
            $text= "Apartment Facility successfully updated";
            $status = true;
            $data = [];
