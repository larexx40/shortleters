        ...
        if($sort > 0){
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                ...
                $searchQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                                ...
                                WHERE (users.fname LIKE ? OR users.lname LIKE ? OR productcart.orderstatus_id LIKE ? OR  productcart.track_id LIKE ? OR productcart.orderref_number LIKE ? OR logistics.name LIKE ? ) AND productcart.`orderstatus_id` = ? AND productcart.`user_id` =? ";
                $stmt= $connect->prepare($searchQuery);
                $stmt->bind_param("ssssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $sortStatus, $userid);
                $stmt->execute();
                ...
                $stmt= $connect->prepare($searchQuery);
                $stmt->bind_param("ssssssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $sortStatus, $userid, $offset, $noPerPage);
                $stmt->execute();
                $result= $stmt->get_result();
                ...
                $stmt->bind_param("ss", $sortStatus, $userid);
                $stmt->execute();
                $result= $stmt->get_result();
                $total_numRow = $result->num_rows;
                $pages = ceil($total_numRow / $noPerPage);
                ...
                $stmt= $connect->prepare($sqlQuery);
                $stmt->bind_param("ssss", $sortStatus,$userid, $offset, $noPerPage);
                $stmt->execute();
                $result= $stmt->get_result();
                $numRow = $result->num_rows;
            }
