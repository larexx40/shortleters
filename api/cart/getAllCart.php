    ...
    if($method =='GET'){
        ...
        if($sort > 0){
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                //search productCategory from database (get total number of row)
                $searchParam = "%{$search}%";
                $searchQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                                ...
                                WHERE (users.fname LIKE ? OR users.lname LIKE ? OR productcart.orderstatus_id LIKE ? OR  productcart.track_id LIKE ? OR productcart.orderref_number LIKE ? OR logistics.name LIKE ? ) AND productcart.`orderstatus_id` = ?";
                $stmt= $connect->prepare($searchQuery);
                $stmt->bind_param("sssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $sortStatus);
                $stmt->execute();
                ...
                $stmt= $connect->prepare($searchQuery);
                $stmt->bind_param("sssssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $sortStatus, $offset, $noPerPage);
                $stmt->execute();
                $result= $stmt->get_result();
                ...
                $sqlQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                        productcart.`orderref_number`, productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, productcart.assigntodeliveryman, productcart.seenbyseller, productcart.`totalpaid`,  productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge
                        FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id 
                        WHERE productcart.`orderstatus_id` = ?";
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                ...
                $searchQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                                productcart.`orderref_number`, productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, productcart.assigntodeliveryman, productcart.seenbyseller, productcart.`totalpaid`,  productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge
                                FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id
                                WHERE (users.fname LIKE ? OR users.lname LIKE ? OR productcart.orderstatus_id LIKE ? OR  productcart.track_id LIKE ? OR productcart.orderref_number LIKE ? OR logistics.name LIKE ? )";
                $stmt= $connect->prepare($searchQuery);
                ...
                $searchQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                                productcart.`orderref_number`, productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, productcart.assigntodeliveryman, productcart.seenbyseller, productcart.`totalpaid`,  productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge
                                FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id
                                WHERE (users.fname LIKE ? OR users.lname LIKE ? OR productcart.orderstatus_id LIKE ? OR  productcart.track_id LIKE ? OR productcart.orderref_number LIKE ? OR logistics.name LIKE ? ) ORDER BY id DESC LIMIT ?,?";
                $stmt= $connect->prepare($searchQuery);
                $stmt->bind_param("ssssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $offset, $noPerPage);
                $stmt->execute();
                $result= $stmt->get_result();
                $numRow = $result->num_rows;  
            }else {
                ...
                $sqlQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                            productcart.`orderref_number`, productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, productcart.assigntodeliveryman, productcart.seenbyseller, productcart.`totalpaid`,  productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge
                            FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id ORDER BY id DESC";
                $stmt= $connect->prepare($sqlQuery);
                $stmt->execute();
                $result= $stmt->get_result();
                $total_numRow = $result->num_rows;
                ...
                $sqlQuery = "SELECT productcart.`id`, productcart.ordertime,productcart.paid, productcart.`user_id`, users.fname AS firstname, users.lname AS lastname, productcart.`orderstatus_id`, productcart.`track_id`,
                            productcart.`orderref_number`, productcart.`logisticid`, logistics.name AS logistic_name, productcart.deliveryaddress_id, productcart.assigntodeliveryman, productcart.seenbyseller, productcart.`totalpaid`,  productcart.`totalweightlbs`,productcart.noofprod_ordered, productcart.delivery_charge
                            FROM `productcart` LEFT JOIN users ON productcart.`user_id` = users.id LEFT JOIN logistics ON productcart.`logisticid` = logistics.id ORDER BY id DESC LIMIT ?,?";
                $stmt= $connect->prepare($sqlQuery);
