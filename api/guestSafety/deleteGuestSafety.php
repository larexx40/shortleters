<?php
    ...
    if($method =='POST'){
        ...
        //delete Guest safety
        $sqlQuery = "DELETE FROM `guest_safety` WHERE `guest_safetyid` =?";
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = ["Guest safety deleted from the database"];
            $errordata = [];
            $text = "Guest safety successfully deleted";
            $status = true;
            $data = returnSuccessArray($text, $method, $endpoint, $errordata, $maindata, $status);
            respondOK($data);
        }else{
            //id not found response
            $errordesc="Guest safety id not found";
            $linktosolve="htps://";
            $hint='Guest safety id not in database';
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Guest safety id not found";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
