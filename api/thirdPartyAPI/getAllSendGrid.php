<?php
    ...
    if ($method == 'GET') {
        ...
        if (!isset($_GET['per_page'])){
            ...
            $noPerPage = cleanme($_GET['per_page']);
        }
        ...
        if (!empty($search) && $search != "" && $search != " "){
            ...
            if ( $status !== "" && $status !== " "){
                ...
                $total_pg_found =  ceil($total_num_row / $noPerPage);

                $query = "SELECT * FROM sendgridapidetails WHERE status = ? AND( name Like ? OR emailfrom Like ?) LIMIT ?, ?";
                $queryStmt = $connect->prepare($query);
                $queryStmt->bind_param("sssss", $status , $searching, $searching, $offset, $noPerPage);
                $queryStmt->execute();
                $resut = $queryStmt->get_result();
                ...
                $queryStmt = $connect->prepare($query);
                $queryStmt->bind_param("ssss", $searching, $searching, $offset, $noPerPage);
                $queryStmt->execute();
                $resut = $queryStmt->get_result();
                $num_row = $resut->num_rows;
            ...
            if ($status !== "" && $status !== " "){
                ...
                $queryStmt = $connect->prepare($query);
                $queryStmt->bind_param("sss", $status ,$offset, $noPerPage);
                $queryStmt->execute();
                $resut = $queryStmt->get_result();
                $num_row = $resut->num_rows;
            }else{
                ...
                $queryStmt = $connect->prepare($query);
                $queryStmt->bind_param("ss", $offset, $noPerPage);
                $queryStmt->execute();
                $resut = $queryStmt->get_result();
                $num_row = $resut->num_rows;
            }
