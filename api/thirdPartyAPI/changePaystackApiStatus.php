<?php
    ...
    if ($method == 'POST') {
        ...
        if ($num_row < 1){
            ...
            $linktosolve = 'https://';
            $hint = "Kindly pass the valid Paystack API id field in this endpoint";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, null);
            respondBadRequest($data);
        ...
        $updateDefault = "UPDATE paystackapidetails SET status = ? WHERE status = ? ";
        ...
        $updateDefault = "UPDATE paystackapidetails SET status = ? WHERE id = ?";
        $updateStmt = $connect->prepare($updateDefault);
