<?php
    ...
    if($method =='GET'){
        ...
        if(empty($id)){
            ...
            //delete with id
            $sqlQuery = "DELETE FROM `paystackapidetails` WHERE `id` =  ?";
            ...
            if($deleteByid){
                ...
                $errordata = [];
                $text = "Paystack Api details successfully deleted";
                $status = true;
                ...
                $errordata=returnError7003($errordesc,$linktosolve,$hint);
                $text="Failled to delete paystack details, fill all require data input";
                $method=getenv('REQUEST_METHOD');
                $data=returnErrorArray($text,$method,$endpoint,$errordata);
