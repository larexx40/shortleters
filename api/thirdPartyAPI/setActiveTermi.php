<?php
    ...
    if ($method == 'POST') {
        ...
        $zero = 0;
        // check query
        $checkQuery = "UPDATE `termiapidetails` SET `status`= ? WHERE status = ?";
        ...
        // Delete the data
        $query = "UPDATE `termiapidetails` SET `status`= ? WHERE id = ?";
        $setActive = $connect->prepare($query);
        $setActive->bind_param("ss", $one ,$termiID );
        

        if ( $setActive->execute()){
            $data = [];
            $text= "Termi Api successfully set as active";
            $status = true;
