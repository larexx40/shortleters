<?php
    ...
    if($method =='GET'){
        ...
        if(empty($id)){
            ...
            $sqlQuery = "DELETE FROM `monifyapidetails` WHERE `id` =  ?";
            ...
            if($deleteByid){
               ...
               $errordata = [];
               $text = "Monify Api details successfully deleted";
               $status = true;
               ...
               $errordata=returnError7003($errordesc,$linktosolve,$hint);
               $text="Failled to delete monify details, fill all require data input";
               $method=getenv('REQUEST_METHOD');
               $data=returnErrorArray($text,$method,$endpoint,$errordata);
