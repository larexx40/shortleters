    ...
    if($method =='GET'){
        ...
        if($sort > 0){
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                ...
                $searchParam = "%{$search}%";
                $searchQuery = "SELECT * FROM `paystackapidetails` WHERE (`name` like ? OR `apikey` like ?) AND status = ?";
                ...
                $sqlQuery = "SELECT * FROM `paystackapidetails` WHERE status = ?";
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                //search for data in database 
                $searchParam = "%{$search}%";
                $searchQuery = "SELECT * FROM `paystackapidetails` WHERE `name` like ? OR `apikey` like ?";
                $stmt= $connect->prepare($searchQuery);
                ...
                $searchQuery = "SELECT * FROM `paystackapidetails` WHERE `name` like ? OR `apikey` like ? ORDER BY id DESC LIMIT ?,?";
                $stmt= $connect->prepare($searchQuery);
                ...
                //get all data in database
                $sqlQuery = "SELECT * FROM `paystackapidetails` ORDER BY id DESC";
                $stmt= $connect->prepare($sqlQuery);
                $stmt->execute();
                $result= $stmt->get_result();
                $total_numRow = $result->num_rows;
                ...
                $sqlQuery = "SELECT * FROM `paystackapidetails` ORDER BY id DESC LIMIT ?,?";
                $stmt= $connect->prepare($sqlQuery);
