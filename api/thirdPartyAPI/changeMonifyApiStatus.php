<?php
    ...
    if ($method == 'POST') {
        ...
        $query = 'SELECT * FROM monifyapidetails WHERE id = ?';
        ...
        if ($num_row < 1){
            ...
            $linktosolve = 'https://';
            $hint = "Kindly pass the valid monify API id field in this endpoint";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, null);
            respondBadRequest($data);
        ...
        $updateDefault = "UPDATE monifyapidetails SET status = ? WHERE status = ? ";
        ...
        $updateDefault = "UPDATE monifyapidetails SET status = ? WHERE id = ?";
        $updateStmt = $connect->prepare($updateDefault);
        $updateStmt->bind_param("ss", $one, $id);
        $updateStmt->execute();
