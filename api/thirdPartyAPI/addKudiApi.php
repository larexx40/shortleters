<?php
    ...
    if($method =='POST'){
        ...
        $status =0;
        if(empty($username) || empty($password) ||empty($name) || empty($sendFrom)){
            ...
            $sqlQuery = "INSERT INTO `kudiapidetails`( `sendfrom`, `username`, `status`, `name`, `password`) VALUES  (?,?,?,?,?)";
            $stmt = $connect->prepare($sqlQuery);
            $stmt->bind_param("sssss",$sendFrom,$username,$status,$name,$password);
            $addApiData=$stmt->execute();
