    ...
    if($method =='GET'){
        ...
        if($sort > 0){
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                ...
                $searchParam = "%{$search}%";
                $searchQuery = "SELECT * FROM `kudiapidetails` WHERE (`name` like ? OR `username` like ? OR `sendfrom` like ?) AND status = ?";
                ...
                //get all data
                $sqlQuery = "SELECT * FROM `kudiapidetails` WHERE status = ?";
            ...
            if (!empty($search) && $search!="" && $search!=' '){
                //search from database 
                $searchParam = "%{$search}%";
                $searchQuery = "SELECT * FROM `kudiapidetails` WHERE `name` like ? OR `username` like ? OR `sendfrom` like ?";
                $stmt= $connect->prepare($searchQuery);
        ...
        if($numRow > 0){
            ...
            while($data = $result->fetch_assoc()){
                ...
                $name = $data['name'];
                array_push($allResponse,array("id"=>$id, "username"=>$username, "name"=>$name, "status"=>$status, "sendfrom"=>$sendfrom));
            }
            $maindata = [
                'page' => $page_no,
                'per_page' => $noPerPage,
