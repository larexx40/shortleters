<?php
    ...
    if ($method == 'GET') {
        ...
        if ( !isset($_POST['user_id'])) {
            ...
            $userId = cleanme($_POST['user_id']);
        }

        if ( !is_numeric($userId) ){
            // Inavlid input
            $errordesc = "Insert a valid user id";
            $linktosolve = 'https://';
        ...
        if (!empty($search) && $search != "" && $search != " "){
            ...
            $getSearchPages->bind_param("ssss", $searching, $searching, $searching, $userId);
            ...
            $getAll->bind_param("ssssss", $searching,  $searching, $searching, $userId ,$offset, $no_per_page);
            $getAll->execute();
            ...
            $query = "SELECT * FROM `tracking_history` WHERE tracking_history.user_id = ?";
            $getSearchPages = $connect->prepare($query);
            ...
            // Output page
            $query = "SELECT * FROM `tracking_history` WHERE tracking_history.user_id = ? LIMIT ?, ?";
            $getAll = $connect->prepare($query);
