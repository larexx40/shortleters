<?php
    ...
    if ($method == 'POST') {
        ...
        $query = "INSERT INTO `tracking_history`(`orderef_no`, `orderstatus`, `detail`, `user_id`, `logistics_id`) VALUES (?, ?, ?, ?, ?)";
        $addTracking = $connect->prepare($query);
        $addTracking->bind_param("sssss", $orderRefNo, $status, $details, $userId, $logistics_id);

        if ( $addTracking->execute() ){
            $data = [];
            $text= "Tracking successfully added";
            $status = true;
