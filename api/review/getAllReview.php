    ...
    if($method =='POST'){
        ...
        if (!empty($search) && $search!="" && $search!=' '){
            ...
            $searchQuery = "SELECT productreview.id, productreview.userid, users.username, productreview.productid, products.name AS product_name, productreview.review, productreview.ratestar 
                            ...
                            WHERE users.username LIKE ? OR products.name LIKE ? OR productreview.review LIKE ?";
            ...
            $searchQuery = "SELECT productreview.id, productreview.userid, users.username, productreview.productid, products.name AS product_name, productreview.review, productreview.ratestar 
                            FROM `productreview` LEFT JOIN users ON productreview.userid = users.id LEFT JOIN products ON productreview.productid = products.id 
                            WHERE (users.username LIKE ? OR products.name LIKE ? OR productreview.review LIKE ?) ORDER BY productreview.id DESC LIMIT ?,?";
            $stmt= $connect->prepare($searchQuery);
            ...
            $sqlQuery = "SELECT productreview.id, productreview.userid, users.username, productreview.productid, products.name AS product_name, productreview.review, productreview.ratestar 
                        FROM `productreview` LEFT JOIN users ON productreview.userid = users.id LEFT JOIN products ON productreview.productid = products.id ORDER BY productreview.id DESC";
            ...
            $sqlQuery = "SELECT productreview.id, productreview.userid, users.username, productreview.productid, products.name AS product_name, productreview.review, productreview.ratestar 
                        FROM `productreview` LEFT JOIN users ON productreview.userid = users.id LEFT JOIN products ON productreview.productid = products.id ORDER BY productreview.id DESC LIMIT ?,?";
            $stmt= $connect->prepare($sqlQuery);
