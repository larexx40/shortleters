<?php
    ...
    if ($method == 'POST') {
        ...
        if ( empty($email) || empty($review) || empty($apartment_id) || !is_numeric($rate_star) ){
        ...
        $review_id = generateUniqueShortKey($connect, "apartment_review", "review_id");


        $query = 'INSERT INTO `apartment_review`(`review_id`, `email`, `userid`, `apartment_id`, `review`, `ratestar`) VALUES (?, ?, ?, ?, ?, ?)';
        $slider_stmt = $connect->prepare($query);
        $slider_stmt->bind_param("ssssss", $review_id, $email, $user, $apartment_id, $review, $rate_star);
