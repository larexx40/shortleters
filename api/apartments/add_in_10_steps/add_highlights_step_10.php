<?php
    ...
    if ($method == 'POST') {
        ...
        if ( !isset($_POST['highlights_ids']) ){

            $errordesc="Hightlight ids is required, Kindly seperate with a comma";
            $linktosolve="htps://";
            $hint=["Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Hightlight id must be passed";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);

        }else{
            $highlights_ids = cleanme($_POST['highlights_ids']);
        }
        ...
        if ( empty($apartment_id) || empty($highlights_ids) ){
        ...
        $hightlight_id_array = explode(",", $highlights_ids);
        $hightlight_length = count($hightlight_id_array);
        ...
        // Make user and agent
        $query = "UPDATE `apartments` SET `highlights_ids`= ?, `steps`= ? WHERE `apartment_id` = ? AND agent_id = ?";
        $updateStatus = $connect->prepare($query);
        $updateStatus->bind_param("ssss", $highlights_ids, $steps ,$apartment_id ,$user_id);
