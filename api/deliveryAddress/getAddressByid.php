<?php
    ...
    if ($method == 'GET') {
        ...
        //check if its admin
        if (!isset($_GET['id'])) {
            $errordesc =  "Address id not passed";
            $linktosolve = 'https://';
            $hint = "pass id of address you want to get";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, null);
            respondBadRequest($data);
        } else {
        ...
        if(empty($id)){
            ...
            $sqlQuery = "SELECT * FROM `deliveryaddress` WHERE userid = ? AND id = ?";
            ...
            if($result->num_rows > 0){
                //pass fetched data into maindata[]
