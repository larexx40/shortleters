<?php
    ...
    if($method =='POST'){
        ...
        if(empty($email) || empty($name) ||empty($username) || empty($password)){
            ...
            $sqlQuery = "INSERT INTO `admin`(`userpubkey`, `email`, `username`, `password`, `name`, `status`, `superadmin`) VALUES  (?,?,?,?,?,?, ?)";
            $stmt = $connect->prepare($sqlQuery);
            $stmt->bind_param("sssssss",$adminPubKey,$email,$username, $hashPassword, $name,$status, $super_admin);
