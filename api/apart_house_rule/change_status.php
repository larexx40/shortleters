<?php
    ...
    if ($method == 'POST') {
        ...
        if ( empty($apart_rule_id) ){
            ...
            $errordesc = "Enter apartment rule id";
            $linktosolve = 'https://';
        ...
        // update status
        $query = "UPDATE `apartment_house_rule` SET `status` = ? WHERE apart_rule_id = ?";
        $updateStatus = $connect->prepare($query);
        $updateStatus->bind_param("ss", $changeStatus, $apart_rule_id);
        ...
        if ( $updateStatus->execute()){
            ...
            $data = [];
            $text= "Apartment House Rule successfully ". $changeStatusText;
            $status = true;
