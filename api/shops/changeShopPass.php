<?php
    ...
    if ($method == 'POST') {
        ...
        if ( $num_row < 1){
            ...
            $linktosolve = 'https://';
            $hint = "Shop is not in the database ensure the user is in the database";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, []);
            respondBadRequest($data);
            ...
            if ( check_pass($currentpassword, $oldHashedPass) ){
                ...
                $updatePassQuery = "UPDATE shops SET password = ? WHERE id = ?";
                $updateStmt = $connect->prepare($updatePassQuery);
                $updateStmt->bind_param('si', $hashNewPassword, $shop_id);
                $updateStmt->execute();
