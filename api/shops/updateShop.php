<?php
    ...
    if ($method == 'POST') {
        ...
        if ( !isset($_POST['shop_username'])) {

            $errordesc="Shop Username is  required";
            $linktosolve="htps://";
            $hint=["Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            ...
            $shop_username = cleanme($_POST['shop_username']);
        }
        ...
        if (empty($shopCountry) || empty($shopCity) || empty($shopPhone) || empty($shopWhatsApp) || empty($shopType) || empty($shopAddress) || empty($minCost)
            || empty($openTime) || empty($closeTime) || empty($description) || empty($shop_username) )  {
        ...
        $Checkquery = "SELECT * FROM `shops` WHERE `id` = ? AND `username`  = ?";
        $Checkstmt = $connect->prepare($Checkquery);

        $Checkstmt->bind_param("ss", $shop_id, $shop_username );
        $Checkstmt->execute();
        $result = $Checkstmt->get_result();
        $num_row = $result->num_rows;

        if ($num_row < 1){
            $response = checkIfLogisticOrShopIsUnique($connect, "shops", "", "", $shop_username);
        ...
        $Checkquery = "SELECT * FROM `shops` WHERE `id` = ? AND `office_phone`  = ?";
        $Checkstmt = $connect->prepare($Checkquery);

        $Checkstmt->bind_param("ss", $shop_id, $shopPhone );
        $Checkstmt->execute();
        $result = $Checkstmt->get_result();
        $num_row = $result->num_rows;
        ...
        $query = "UPDATE `shops` SET country= ?,address= ?,city= ?,accept_order= ?,min_cost= ?,open_time= ?,close_time= ?,office_phone= ?,office_whatapp= ?,description= ?,openstatus= ?, shoptype = ?, username = ? WHERE id = ?";
        $updateShop = $connect->prepare($query);
        $updateShop->bind_param("ssssssssssssss", $shopCountry, $shopAddress, $shopCity, $acceptOrder, $minCost, $open_time, $close_time, $shopPhone, $shopWhatsApp, $description, $openStatus, $shopType, $shop_username ,$shop_id );
