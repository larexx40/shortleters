<?php
    ...
    if ($method == 'GET') {
        ...
        $query = "DELETE FROM `shop_locations` WHERE id = ?";
        $deleteLoc = $connect->prepare($query);
        $deleteLoc->bind_param("s", $shopLocationId);

        if ( $deleteLoc->execute() ){
