<?php
    ...
    if ($method == 'POST') {
        ...
        if ( !is_numeric($smartID) ){
            ...
            $hint=["Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Kindly pass a valid smart solution id";
            $method;
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
        ...
        // Delete the data
        $query = "DELETE FROM `smartsolutionapidetails` WHERE id = ?";
        $delete = $connect->prepare($query);
        $delete->bind_param("s", $smartID );
        

        if ( $delete->execute()){
            $data = [];
            $text= "Smart Solution Api successfully Deleted";
            $status = true;
