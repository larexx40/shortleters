<?php
    ...
    if ($method == 'GET') {
        ...
        if(empty($id)){
            ...
            $squlQuery = "SELECT * FROM `monifyapidetails` WHERE `id`= ?";
            ...
            if($result->num_rows > 0){
                ...
                $status = $row['status'];
                $name = $row['name'];
                $apimerchant = $row['apimerchant'];
                $apiwallet = $row['apiwallet'];
                $apiaccno = $row['apiaccno'];
                array_push($response,array("secretekey"=>$secretekey, "apikey"=>$apikey, "status"=>$status, "name"=>$name, "apimerchant"=>$apimerchant, "apiwallet"=>$apiwallet, "apiaccno"=>$apiaccno));
