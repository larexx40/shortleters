<?php
    ...
    if($method =='POST'){
        ...
        if (!empty($search) && $search!="" && $search!=' '){
            ...
            $searchParam = "%{$search}%";
            $searchQuery = "SELECT * FROM `paystackapidetails` WHERE `name` like ? OR `apikey` ORDER BY id DESC";
            ...
            $searchQuery = "SELECT * FROM `paystackapidetails` WHERE `name` like ? OR `apikey` ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($searchQuery);
        ...
        if($numRow > 0){
            ...
            while($data = $result->fetch_assoc()){
                ...
                $status = $data['status'];
                $name = $data['name'];
                array_push($allResponse,array("secretekey"=>$secretekey, "apikey"=>$apikey, "name"=>$name, "status"=>$status, ));
            }
