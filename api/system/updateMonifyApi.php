<?php
    ...
    if ($method == 'POST') {
        ...
        //UPDATE `monifyapidetails` SET `id`=,`secretekey`=,`apikey`=,`status`=,`name`=,`apimerchant`=,`apiwallet`=,`apiaccno`= WHERE 1
        if(!isset($_POST['id'])){
        ...
        if(empty($name) || empty($id) || empty($apiWallet) || empty($apiAccno) || empty($secreteKey)|| empty($apiKey)|| empty($apiMerchant)  ){
            ...
            // UPDATE `monifyapidetails` SET `id`=,`secretekey`=,`apikey`=,`name`=,`apimerchant`=,`apiwallet`=,`apiaccno`= WHERE 1
            $sql = "UPDATE `monifyapidetails` SET `secretekey`= ?,`apikey`= ?,`name`=' ?,`apimerchant`= ?, `apiwallet` = ?, `apiaccno` = ? WHERE id = ?";
            $stmt = $connect->prepare($sql);
            $stmt->bind_param('sssssss', $secreteKey, $apiKey, $name, $apiMerchant,$apiWallet, $apiAccno, $id);
            $stmt->execute();
            if($stmt->execute()){
                $maindata=[];
                $errordesc = " ";
