<?php
    ...
    if ($method == 'POST') {
        ...
        $status = 0;
        // Insert the data
        $query = "INSERT INTO `termiapidetails`(`sendfrom`, `apikey`, `status`, `name`, `smstype`, `smschannel`) VALUES (?, ? , ? , ?, ?, ?)";
        $insert = $connect->prepare($query);
        $insert->bind_param("ssssss", $sendFrom, $apiKey, $status, $apiName, $smsType, $smsChannel );

        if ($insert->execute()){
            $data = [];
            $text= "Termi Api successfully Added";
            $status = true;
