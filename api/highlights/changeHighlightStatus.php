<?php
    ...
    if ($method == 'POST') {
       ...
       if(!isset($_POST['highlightid'])){
        ...
        $sql = "UPDATE `highlights` SET `status`= ? WHERE `highlightid` = ?";
        $stmt = $connect->prepare($sql);
        $stmt->bind_param('ss', $changeStatus, $highlightid);
        $stmt->execute();
        ...
        if ( $stmt->affected_rows > 0 ){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = "Highlight status set to $message";
            $errordata = [];
            $text = "Highlight set to $message";
            $status = true;
            ...
            //Highlight not found
            $errordesc = "Highlight with id not found";
            $linktosolve = 'https://';
