<?php
    ...
    if ($method == 'POST') {
        ...
        $status =0;
        $highlightid = generateUniqueShortKey($connect,'highlights','highlightid');

        $query = "INSERT INTO `highlights`(`highlightid`, `icon`, `name`, `status`) VALUES (?,?,?,?)";
        $stmt = $connect->prepare($query);
        $stmt->bind_param("ssss", $highlightid, $icon, $name, $status);
