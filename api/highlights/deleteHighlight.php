<?php
    ...
    if($method =='POST'){
        ...
        //delete Highlight
        $sqlQuery = "DELETE FROM `highlights` WHERE `highlightid` =?";
        ...
        if($affectedRow > 0){
            ...
            $errordesc = " ";
            $linktosolve = "htps://";
            $hint = ["Highlight deleted from the database"];
            $errordata = [];
            $text = "Highlight successfully deleted";
            $status = true;
            $data = returnSuccessArray($text, $method, $endpoint, $errordata, $maindata, $status);
            respondOK($data);
        }else{
            //id not found response
            $errordesc="Highlight id not found";
            $linktosolve="htps://";
            $hint='Highlight id not in database';
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Highlight id not found";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
