<?php
    ...
    if ($method == 'POST') {
        ...
        if (!isset($_POST['email'])){
            $errordesc = "Email must be passed";
            $linktosolve = 'https://';
        ...
        if (!isset($_POST['firstname'])){
            $errordesc = "Firstname must be passed";
            $linktosolve = 'https://';
        ...
        if (!isset($_POST['lastname'])){
            $errordesc = "Lastname must be passed";
            $linktosolve = 'https://';
        ...
        if (!isset($_POST['password'])){
            $errordesc = "Password must be passed";
            $linktosolve = 'https://';
        ...
        if ( $emailExist ){
            ...
            $linktosolve = 'https://';
            $hint = "Email already in database";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, []);
            respondBadRequest($data);
        }
        ...
        // check if email is unique
        $phonenoExist = checkIfExist($connect, "users" , "phoneno" , $phone);
        if ( $phonenoExist ){
            $errordesc = "Phoneno already exist";
            $linktosolve = 'https://';
            $hint = "Phoneno already in database";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, []);
            respondBadRequest($data);
        }
