<?php
    ...
    if ($method == 'POST') {
        ...
        if ( empty($productId) || empty($productName) || empty($productType) || empty($productMadeIn) || empty($productWeight))  {
        ...
        if (!checkifFieldExist($connect, "products" , "productid" , $productId)){
            $errordesc = "Product Not Found";
            $linktosolve = 'https://';
            $hint = "Kindly pass valid id of the product";
            $errorData = returnError7003($errordesc, $linktosolve, $hint);
            $data = returnErrorArray($errordesc, $method, $endpoint, $errorData, []);
            respondBadRequest($data);
        }
        ...
        $query = "UPDATE `products` SET `name`= ?,`type`= ?,`purchaseprice`= ?,`sellingprice`= ?,`quantityavailable`= ?,`category_id`= ?,`subcat_id`= ?,`ddesc`= ?,`special_price`= ?,`discountquantity`= ?,`discountprice`= ?,`madein`= ?,`weight`= ?,`brand_id`= ? WHERE `productid` = ? AND shop_id = ?";
        $addProduct = $connect->prepare($query);
        $addProduct->bind_param("ssssssssssssssss", $productName, $type, $costPrice, $sellingPrice, $productQty, $productCategory, $productSubCategory, $description, $specialPrice, $discountQty, $discountPrice, $productMadeIn, $productWeight, $productBrand, $productId, $shop_id);
