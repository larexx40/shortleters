<?php
    ...
    if($method =='GET'){
        ...
        //check if its admin
        $shop = checkIfShopOwner($connect, $userpubkey);
        
        if(!checkIfIsAdmin($connect, $userpubkey) && !$shop){
            //respond not admin
            $errordesc =  "User not Authorized";
            $linktosolve = 'https://';
            $hint = "Only Admin and shop can access this route";
        ...
        if($numRow > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                array_push($allResponse, array(
                    "id"=>$id,
                    "num_of_sub_product" => $num_of_sub_product,
