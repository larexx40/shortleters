<?php
    ...
    if($method =='GET'){
        ...
        if (!empty($search) && $search!="" && $search!=' '){
            ...
            $searchParam = "%{$search}%";
            $searchQuery = "SELECT productbrand.*, shops.name AS shop_name FROM `productbrand` LEFT JOIN shops ON productbrand.shop_id = shops.id WHERE productbrand.name like ? OR shops.name LIKE ?";
            ...
            $searchQuery = "SELECT productbrand.*, shops.name AS shop_name FROM `productbrand` LEFT JOIN shops ON productbrand.shop_id = shops.id WHERE productbrand.name like ? OR shops.name LIKE ? ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($searchQuery);
            ...
            //get all data
            $sqlQuery = "SELECT * FROM `productbrand` ORDER BY id DESC";
            ...
            $sqlQuery = "SELECT * FROM `productbrand` ORDER BY id DESC LIMIT ?,?";
            $stmt= $connect->prepare($sqlQuery);
        ...
        if($numRow > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                $name = $row['name'];
                $shop_id = $row['shop_id'];
                $shopname = getShopname($connect, $shop_id);
                $num_of_products = checkifFieldExist($connect, "products", "subcat_id", $id);
                array_push($allResponse, array("id"=>$id, "name"=>$name, 'num_of_products' => $num_of_products ,"shop_id" => $shop_id, "shopname" => $shopname));
            }
            $maindata = [
                'page' => $page_no,
                'per_page' => $noPerPage,
