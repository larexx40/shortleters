<?php
    ...
    if ($method == 'POST') {
        ...
        if ( empty($currency_id) ){
            $errordesc = "Currency id must be filled";
            $linktosolve = 'https://';
        ...
        $query = 'DELETE FROM `listing_currency` WHERE `currency_id` = ?';
        $slider_stmt = $connect->prepare($query);
        $slider_stmt->bind_param("s", $currency_id);
        $slider_stmt->execute();
