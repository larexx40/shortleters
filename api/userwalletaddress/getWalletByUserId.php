<?php
    ...
    if ($method == 'GET') {
        ...
        WHERE
            users.lname LIKE ? OR users.fname LIKE ? OR coinproducts.name LIKE ? OR memo LIKE ? AND userwalletaddresses.userid = ? ";
            $getSearchPages = $connect->prepare($query);
            $getSearchPages->bind_param("sssss", $searching, $searching, $searching, $searching, $user_id);
            ...
            $getAll->bind_param("sssssss", $searching,  $searching, $searching, $searching, $user_id ,$offset, $no_per_page);
            $getAll->execute();
        ...
        userwalletaddresses.userid = ?";
            $getSearchPages = $connect->prepare($query);
            $getSearchPages->bind_param("s", $user_id);
            $getSearchPages->execute();
            $result = $getSearchPages->get_result();
            $total_num_row = $result->num_rows;
            $total_pg_found =  ceil($total_num_row / $no_per_page);  
